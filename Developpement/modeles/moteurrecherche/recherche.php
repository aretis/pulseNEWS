...
if ((isset($_POST['recherche']) ||(isset($_GET['recherche']))))
{
	...
	if($mode == "exp_exacte") 
	{
		...
		else 
		{
			...
			{ 
				...
				<a href = "lire_article.php?id='.$resultats['id'].'&recherche='.$recherche.'&mode='.$mode.'">lire l article!</a> </br>';
			}
	...
	{ 
		...
		else 
		{
			...
			{ 
				echo 'titre: '.$resultats['title'].'   Date :'.$resultats['post_date'].'
				<a href = "lire_article.php?id='.$resultats['id'].'&recherche='.$recherche.'&mode='.$mode.'">lire l article!</a>';
			}
